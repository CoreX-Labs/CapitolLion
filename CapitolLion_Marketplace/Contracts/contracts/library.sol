...
library EnumerableMap {
    ...
    function set(
        ...
        if (tokenIdIndex == 0) {
            // Equivalent to !contains(map, tokenId)
            map._entries.push(
                MapEntry({_tokenId: _TokenIdInBytes, _owner: _ownerInBytes})
            );
    ...
    function remove(UintToAddressMap storage map, uint256 tokenId) internal {
        ...
        require(
            tokenIdIndex != 0,
            "EnumerableMap: remove tokenId is nonexistent"
        );
    ...
    function get(UintToAddressMap storage map, uint256 tokenId)
        internal
        view
        returns (address)
    {
...
library EnumerableSet {
    ...
    function add(UintSet storage set, uint256 tokenId) internal {
        require(
            !_contains(set, tokenId),
            "EnumerableSet: tokenId already contain"
        );
    ...
    function remove(UintSet storage set, uint256 tokenId) internal {
        require(
            _contains(set, tokenId),
            "EnumerableSet: tokenId not belongs to owner"
        );
        // We read and store the tokenId's index to prevent multiple reads from the same storage slot
     ...
     */
    function _contains(UintSet storage set, uint256 tokenId)
        private
        view
        returns (bool)
    {
     ...
     * @dev Returns true if the tokenId is in the set. O(1).
     */
    function contains(UintSet storage set, uint256 tokenId)
        internal
        view
        returns (bool)
    {
