...
contract MinterRole is Context {
    ...
    function _changeOwner(address account) internal {
        require(
            account != address(0) && account != ownerAddr,
            "MinterRole: Address is Owner or zero address"
        );
        ownerAddr = account;
...
contract FundController is Context, MinterRole {
        ...
        returns (bool)
    {
        require(
            ...
            "FundController: insufficient balance"
        );

        (bool success, ) = to.call{value: value}("");
        require(
            success,
            "FundController: unable to send value, accepter may have reverted"
        );
        return true;
    }
    ...
    */
    ...
    //     require(
    //         value <= address(this).tokenBalance(token),
    //         "FundController: Not enough Token Available"
    //     );
    ...
    */
    function claimTRC20(
        ...
        require(
            result,
            "FundController: unable to transfer value, recipient or token may have reverted"
        );
        return true;
    }
