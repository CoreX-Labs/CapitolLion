...
contract ICPNFT is Context, TRC165, IKraftNft, BlockRole {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId), "ICPNFT: URI query for nonexistent token");
    ...
    function balanceOf(address owner) public view returns (uint256) {
        require(
            owner != address(0),
            "ICPNFT: balance query for the zero address"
        );
    ...
    function ownerOf(uint256 tokenId) public view returns (address) {
        return
            _tokenOwners.get(
                tokenId,
                "ICPNFT: owner query for nonexistent token"
            );
    }
    ...
    {
        require(
            index < _holderTokens[owner].length(),
            "ICPNFT: owner index out of bounds"
        );
    ...
    function tokenByIndex(uint256 index) public view returns (uint256) {
        require(index < totalSupply(), "ICPNFT: global index out of bounds");
        ...
        isNotBlackListed(_msgSender())
    {
        ...
        require(to != owner, "ICPNFT: approval to current owner");

        require(
            _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
            "ICPNFT: approve caller is not owner nor approved for all"
        );
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
        require(
            _exists(tokenId),
            "ICPNFT: approved query for nonexistent token"
        );
    ...
    function setApprovalForAll(address to, bool approved)
        ...
        isNotBlackListed(_msgSender())
    {
        require(to != _msgSender(), "ICPNFT: approve to caller");
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public isNotPaused isNotBlackListed(to) isNotBlackListed(from) {
        //solhint-disable-next-line max-line-length
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "ICPNFT: transfer caller is not owner nor approved"
        );
    ...
    function _safeTransfer(
        ...
        require(
            ...
            "ICPNFT: transfer to non TRC721Receiver implementer"
        );
    }
        ...
        returns (bool)
    {
        require(
            _exists(tokenId),
            "ICPNFT: operator query for nonexistent token"
        );
        address owner = ownerOf(tokenId);
    ...
    function _safeMint(
        ...
        require(
            ...
            "ICPNFT: minting to non TRC721Receiver implementer"
        );
    }
    ...
    function _mint(address to, uint256 tokenId) internal {
        require(to != address(0), "ICPNFT: mint to the zero address");
        require(!_exists(tokenId), "ICPNFT: token already minted");
    ...
    function _burn(uint256 tokenId, address owner) internal {
        require(
            ...
            "ICPNFT: burn of token that is not own"
        );
    ...
    function _transfer(
        ...
        require(
            ...
            "ICPNFT: transfer of token that is not own"
        );
        require(to != address(0), "ICPNFT: transfer to the zero address");
...
contract CAPITOLNft is Context, MinterControl, ICPNFT {
    using SafeMath for uint256;
    // total number of nft minted in series
    uint256 private serialCount;

    constructor() public ICPNFT("CAPITOL NFT", "KNFT") {
        serialCount = 1;
    }
     ...
     * @param to The address that will own the minted token
     */
    function serialMint(address to)
        public
        onlyMinter
        isNotPaused
        returns (uint256)
   ...
   */
    function serialMint(address to, string memory tokenURI)
        public
        onlyMinter
        isNotPaused
        returns (uint256)
    ...
    ) public onlyMinter isNotPaused returns (bool) {
        _mint(to, tokenId);
